...
## 2026-07-14 (21) — Fix critique: race condition cloudPull / push debounce (clients disparaissent / reviennent)
- **Bug corrigé** : suppression d'un client pouvait le faire revenir après retour sur l'onglet; un client ajouté pouvait disparaître après refresh. Cause racine: **race condition** entre `cloudPull()` (déclenché par `visibilitychange`) et le timer de push debounce (1.5 s). Le timer pouvait s'exécuter pendant le fetch Supabase du pull, mettre à jour `baseIds` dans localStorage, puis le pull finissait et écrasait l'état avec des `baseIds` stales issus du remote — rendant la suppression invisible au prochain pull.
- **Fix — 3 couches de défense** : (1) **Guard de concurrence** `Cloud.pulling` — un second `cloudPull()` lancé pendant un pull actif est silencieusement ignoré (protection `visibilitychange` rapid-fire). (2) **Annulation du push debounce** au début de chaque pull (`clearTimeout(Cloud.pushTimer)`) — garantit qu'aucun push timer ne peut s'interposer pendant le fetch et corrompre les `baseIds`. (3) **`try/catch/finally`** — `Cloud.pulling` est toujours réinitialisé même en cas d'erreur réseau. Le push des changements locaux est délégué au `cloudPush(true)` en fin de pull (chemin existant, inchangé).
- Logique de merge, LWW, `baseIds`, `sweepOrphans` : **inchangés**. SW v34.
