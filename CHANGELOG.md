...
## 2026-07-14 (26) — SyncEngine v2: réécriture de la couche persistance/sync (app inchangée)
- **La base LOCALE est l'unique source de vérité. Le cloud n'écrase JAMAIS le local.** Réécriture complète de la seule couche instable — UI, pages, Dashboard, rendu, calculs, thème, icônes: **inchangés**.
- **Nouveaux modules** (5): `idb.js` (wrapper IndexedDB), `storage.js` (base `THEY_STUDIO_DB`, stores clients/projets/taches/paiements/events/queue/settings/sync_state; **soft-delete** `deletedAt`; `updatedAt`/`version`/`deviceId` par ligne; diff automatique; migration one-shot localStorage→IDB; BroadcastChannel multi-onglets), `queue.js` (file CREATE/UPDATE/DELETE persistante, coalescing), `sync-engine.js` (pull→merge→persist→render→push en arrière-plan, single-flight), `auth-sync.js` (login qui ne touche jamais au local).
- **Flux d'écriture**: UI → IndexedDB → render immédiat → file → sync en arrière-plan → Supabase. Jamais d'attente réseau.
- **Conflits**: `deletedAt` l'emporte sur des updates plus anciens; sinon `updatedAt` le plus récent gagne; `version` incrémentée à chaque modif; **aucune ligne ne disparaît en silence** (création locale absente du cloud → ré-enfilée, jamais supprimée).
- **DELETE**: soft-delete + tombstone poussé; **plus AUCUN DELETE physique**. Migration SQL: `supabase/migration-tombstones.sql` (colonnes `deleted_at/version/device_id`).
- **Supprimé**: tout le moteur legacy (`cloudPull/cloudPush/baseIds/mergeFromDisk/cloudStamp/cloudBoot`, storage events, WLOG/TRACE, jeton de génération, safeSet legacy) — ~28k caractères retirés de `index.html`.
- **Tests**: `tests/sync-v2.js` (38 ✓ — + purge tombstones après confirmation serveur, résilience retry/backoff), `tests/app-regression.js` (17 ✓ — timezone/orphelins/seed/restore/multi-onglets re-branchés), `tests/acceptance.js` (3 runs 25 ✓). ANCIEN placeholder (28 ✓ — local, login sans écrasement, 2 appareils, soft-delete sans résurrection, offline→online, conflits updatedAt & delete-vs-edit, anti-résurrection, reprise de session) + `tests/acceptance.js` (scénario exact create/refresh/delete/refresh ×3, 4 environnements: **3 runs consécutifs 25 ✓/0 ✗**). SW v38 (précache les 5 modules).
- Note: les régressions non-cloud héritées (timezone, orphelins, restore, seed — code conservé) restent à re-brancher sur la nouvelle base de tests dans un sprint dédié.

## 2026-07-14 (25) — FIX: écritures locales fiables (quota) + stabilisation multi-instance
- **Procédure exécutée point par point** : (1) save()→localStorage: les `setItem` échouaient EN SILENCE quand le stockage est plein (images base64 + snapshots + journaux accumulés) → créations perdues, et `they_sync_v1` jamais persisté → baseIds vide → le pull ré-adoptait TOUT le cloud (« les supprimés reviennent tous »). **Fix `safeSet()`** : écriture → si échec, libération des clés sacrifiables (journaux/snapshots/secours) → réécriture → **vérification par relecture** ; échec final = bannière BLOQUANTE rouge (plus jamais silencieux). `save()` n'envoie RIEN au cloud si l'écriture locale a échoué (zéro divergence local/cloud). Hygiène au boot: journaux bornés.
- (2) cloudPush: en cas d'échec en cours de push, la progression réelle des baseIds est persistée (le cloud et baseIds ne divergent plus après un échec partiel, ex. table meta). (3) cloudPull: LWW inchangé, vérifié. (4) baseIds: mis à jour à chaque push réussi + progression persistée sur échec. (5) SW: v37, recharge toutes les fenêtres à l'activation (aucun ancien build ne survit).
- **Retrait du « jeton de génération »** introduit dans la journée: faux positifs entre pairs équipés sous tempête d'events storage (source d'instabilité démontrée par test). La défense multi-instance = mergeFromDisk + sync storage active + SW purge (suffisant, prouvé).
- **Critères d'acceptation** (create→refresh→existe · delete→refresh→supprimé, ×3) : **3 runs complets consécutifs 25 ✓ / 0 ✗** en 4 environnements — local · cloud · HOSTILE (2ᵉ instance ouverte sauvegardant après chaque étape) · QUOTA-HOSTILE (storage saturé). Suite: 112 ✓. SW v37.
