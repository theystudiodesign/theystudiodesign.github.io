...
## 2026-07-14 (21) — Fix critique: race condition + localStorage quota (clients disparaissent / reviennent)
- **Cause racine identifiée** : double bug. (A) `writeSyncState()` et `DataLayer.write()` échouent silencieusement quand le localStorage est plein (images facture base64). Conséquence: `baseIds` n'inclut pas les nouveaux clients → si supprimé localement, le push ne les retire pas de Supabase → le prochain pull les voit dans le remote sans les trouver dans `baseIds` → les traite comme "ajouté par un autre appareil" → **les remet**. (B) Race condition entre `cloudPull()` et le timer debounce du push (1.5 s).
- **Fix 1 — Résilience quota localStorage** : `writeSyncState()` et `DataLayer.write()` : en cas de `QuotaExceededError`, libèrent automatiquement les snapshots (backups rotatifs, non critiques pour le sync temps-réel) et réessaient. `they_snap_2` d'abord, puis `they_snap_1` si nécessaire. Si encore plein → erreur visible dans la console.
- **Fix 2 — Race condition cloudPull** : (1) Guard `Cloud.pulling` — pull concurrent ignoré. (2) `clearTimeout(Cloud.pushTimer)` au début du pull — timer ne peut plus s'interposer pendant le fetch. (3) `try/catch` — flag toujours libéré en cas d'erreur réseau.
- **Warning amélioré** : message `save()` plus explicite + `console.error` quand la donnée n'est vraiment pas sauvegardée. SW v35.
