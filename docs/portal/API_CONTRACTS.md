...
| Updates feed | `GET /rest/v1/v_client_updates?portal_project_id=eq.{id}&order=published_at.desc` | `[{id,title,body_md,published_at}]` |
...
|---|---|---|
| Approve / request changes | `POST /rest/v1/approvals` `{deliverable_id, decision, note?}` | policy: role=client_owner ∧ member ∧ deliverable.status='shared'; `decided_by` forced to `auth.uid()`; `changes_requested` requires non-empty note (CHECK) |
...
```
sign_download(deliverable_id uuid) → { url text, expires_at timestamptz }
...
portal_summary()                  → { projects int, unread int, invoices_due int, next_meeting timestamptz }
  one round-trip for the dashboard header
```
Errors: `P0001 not_a_member`, `P0002 not_available` → UI maps to calm empty/expired states.

## 4. Edge Functions

| Function | Auth | Contract |
|---|---|---|
| `invite` | studio or client_owner JWT (verified inside) | `POST {email, client_id, role}` → `201 {member_id}` · `409 already_member` · `403 role_not_allowed` (owner may only create client_member) |
| `calendly-webhook` | Calendly HMAC signature | `POST` invitee.created/canceled → upsert `bookings` (idempotent on event uri) → `204` |
| `notify` | cron (Supabase scheduled) | batches `notifications` where `read_at is null and emailed_at is null and created_at < now()-'15 min'` → Resend digest → stamps `emailed_at` |

## 5. Realtime

Channel: `notifications:user_id=eq.{uid}` (INSERT) → badge increment; `deliverables` & `updates` changes scoped per open project page. Reconnect strategy: on `visibilitychange`, refetch the affected lists (no client cache to reconcile — server state only).
