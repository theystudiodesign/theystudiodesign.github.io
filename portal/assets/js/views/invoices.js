...
export async function render(outlet) {
  ...
  if (!invoices.length) { wrap.innerHTML = '<div class="empty rise"><p class="h3">${t("No invoices yet.")}</p></div>'; return; }

  const open = invoices.filter((i) => i.status === "sent" || i.status === "overdue").length;
  const paid = invoices.filter((i) => i.status === "paid").length;
  wrap.appendChild(el(`<div class="rise" style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px"><span class="label section-label">${t("Invoices")}</span><span class="mono">${open} open · ${paid} paid</span></div>`));
  ...
  invoices.forEach((i) => {
    ...
    row.querySelector("button").addEventListener("click", async (e) => {
      ...
      try { const { url } = await api.signInvoice(i); window.open(url, "_blank", "noopener"); }
